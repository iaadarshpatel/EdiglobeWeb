...
const AllEmployee = () => {
  ...
  const totalEmployee = filteredUserData.length;
  const currentlyWorkingCount = filteredUserData.filter(
    (employee) => employee.working_status === 'Currently Working'
  ).length;
  const departedCount = filteredUserData.filter(
    (employee) => employee.working_status === 'Departed'
  ).length;
  const noticePeriodCount = filteredUserData.filter(
          ...
          <div className="row">
            {loading ? (
              ...
              />
                ...
                <button className={`badge fs-6 w-auto mt-2 p-3 text-dark ${
                    working_status === "Currently Working"
                      ? "bg-success bg-opacity-25"
                      : working_status === "Departed"
                      ? "bg-danger bg-opacity-25"
